              ...
              <div className="hidden md:flex items-center space-x-1">
                  {navLinks.map(link => (
                      <a 
                        ...
                        className="text-gray-600 hover:bg-gray-100 hover:text-gray-900 px-3 py-2 rounded-md text-sm font-medium cursor-pointer"
                      >
                          {link.name}
                      </a>
                  ))}
                  {loggedIn && (
                    <a onClick={() => handleTabChange('profile')} className="text-gray-600 hover:bg-gray-100 hover:text-gray-900 px-3 py-2 rounded-md text-sm font-medium cursor-pointer">
                        Profile
                    </a>
                  )}
              </div>
              ...
              <div className="md:hidden px-2 pt-2 pb-3 space-y-1 sm:px-3">
                  {navLinks.map(link => (
                      <a 
                        key={link.tab} 
                        onClick={() => link.tab === 'report' ? handleReportClick() : handleTabChange(link.tab)} 
                        className="text-gray-600 hover:bg-gray-100 block px-3 py-2 rounded-md text-base font-medium cursor-pointer"
                      >
                          {link.name}
                      </a>
                  ))}
                   {loggedIn && (
                    <a onClick={() => handleTabChange('profile')} className="text-gray-600 hover:bg-gray-100 block px-3 py-2 rounded-md text-base font-medium cursor-pointer">
                        Profile
                    </a>
                  )}
